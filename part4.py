    ...
    while video.isOpened():
        ...
        if ret:  # check whethere the frame is legal, i.e., there still exists a frame
            ...
            if len(good) > MIN_MATCH_COUNT:
                src_pts = np.float32([kp_m[m.queryIdx].pt for m in good]).reshape(
                    -1, 1, 2
                )
                dst_pts = np.float32([kp_f[m.trainIdx].pt for m in good]).reshape(
                    -1, 1, 2
                )
            ...
            if debug:
                ...
                img_match = cv2.drawMatches(
                    template, kp_m, frame, kp_f, good, None, **draw_params
                )

                print("#good matches: %d" % (len(good)))
                cv2.imwrite(
                    "./debug/{}/match_{:03d}_{:03d}.jpg".format(
                        DET_METHOD, i, len(good)
                    ),
                    img_match,
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--video_path", dest="video_path", default="./input/ar_marker.mp4"
    )
